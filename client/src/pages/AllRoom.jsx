...
const AllRoom = () => {
    ...
    // Handel changes for filters and sorting
  ...
  const matchesPriceRange = (room) => {
        ...
        return selectedFilters.priceRange.some(range => {
            const [min, max] = range.split('to').map(s => Number(s.trim()))
   ...
   const sortRooms = (a, b) => {
    ...
    // filter and sort room based on selected filters and sort option 
        ...
        <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
            <div>
                ...
                {filteredRoom.map((room) => (
                    <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-none 
                       group hover:bg-gray-300 hover:shadow-xl hover:rounded-2xl p-4 -mx-4 transition-all duration-500 ease-out hover:-translate-y-1'>
                        <div className="relative overflow-hidden rounded-xl shadow-lg group-hover:shadow-2xl transition-all duration-500">
                            <img
                                ...
                                src={room.images[0]} alt="hotel-img" title='View Room Details' className='max-h-65 wd:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer' />

                            <div className="absolute inset-0 -translate-x-[150%] group-hover:translate-x-[150%] 
                    transition-all duration-700 ease-out 
                    bg-gradient-to-r from-transparent via-white/40 via-50% to-transparent" />
                        </div>
                        <div className='md:w-1/2 flex flex-col gap-2'>
                            <p className='text-gray-500'>{room.hotel.city}</p>
                            <p onClick={() => { navigate(`/rooms/${room._id}`); window.scrollTo(0, 0) }} className='text-gray-800 text-3xl font-playfair cursor-pointer hover:text-orange-600 transition-colors'>{room.hotel.name}</p>
                            ...
                            <div className='flex flex-wrap items-center mt-3 mb-6 gap-4' >
                                {room.amenities.map((item, index) => (
                                    <div key={index} className='flex items-start gap-2 px-3 py-2 rounded-lg bg-[#F5F5FF]/70' >
