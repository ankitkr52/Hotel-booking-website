...
const RoomDetails = () => {
  ...
  const checkAvailability = async () => {
    try {
      ...
      const { data } = await axios.post('/api/bookings/check-availability', { roomId: id, checkInDate, checkOutDate })
      if (data.success) {
        if (data.isAvailable) {
          setIsAvailable(true)
  ...
  const onsubmitHandler = async (e) => {
    try {
      ...
      if (!isAvailable) {
        ...
        const { data } = await axios.post('/api/bookings/book', { roomId: id, checkInDate, checkOutDate, guests, paymentMethod: 'Pay at Hotel' }, { headers: { Authorization: `Bearer ${await getToken()}` } })
        if (data.success) {
